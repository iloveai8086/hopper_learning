...
__device__ void MMA_SP_WRAPPER(uint32_t * c, GmmaDescriptor desc_a, GmmaDescriptor desc_b, uint32_t metadata) {
	asm volatile("wgmma.mma_async.sp.sync.aligned.m64n32k32.f16.f16.f16 "
				 "{%0, %1, %2, %3, %4, %5, %6, %7}, " // c
				 "%8, %9, "	  // desc A, B
				 "%10, "		  // meta
				 "%11, "		  // thread selection
...
__global__ void kernel(
	...
	// divide the 256x64 of A into 4 64x32 tiles and multiply them with the B divided into 2 32x32 tiles
	// accumulator
	uint32_t c[4][8] = {};
	
	desc_b = make_desc<half *, 8, 16, 2>(B_shared);
	#pragma unroll
	...
	desc_b = make_desc<half *, 8, 16, 2>(B_shared + 32 * N);
	#pragma unroll
	for (int m2 = 0; m2 < 4; m2++) {
		warpgroup_arrive();
	...
	for (int m2 = 0; m2 < 4; m2++) {
		...
		C_ptr[offset1 + 4 + 4] = c[m2][4];
		C_ptr[offset2 + 4 + 4] = c[m2][5];
		C_ptr[offset1 + 4 + 4 + 4] = c[m2][6];
		C_ptr[offset2 + 4 + 4 + 4] = c[m2][7];
	}
}
...
int main() {
	...
	CUtensorMap tensor_map_b = create_2d_tensor_map<half, CU_TENSOR_MAP_DATA_TYPE_FLOAT16, CU_TENSOR_MAP_SWIZZLE_64B>(K, N, K, N, d_B);
