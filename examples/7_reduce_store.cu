...
__global__ void kernel(const __grid_constant__ CUtensorMap tensor_map,
	...
	if (threadIdx.x == 0) {
		...
		// with thet tile in the global memory and shared memory and store the
		// result back to global memory tile
		asm volatile("cp.reduce.async.bulk.tensor.1d.global.shared::cta.add."
					 "tile.bulk_group "
					 "[%0, {%1}], [%2];\n"
					 :
